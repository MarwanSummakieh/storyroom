...
function findChapter(data: StoryroomData, chapterId: string) {
  for (const novel of data.novels) {
...
function findStoryBibleEntry(data: StoryroomData, entryId: string) {
  for (const novel of data.novels) {
    const entry = novel.storyBible.find((item) => item.id === entryId);
    if (entry) {
      return { novel, entry };
    }
  }
...
export async function updateNovel(
  ...
  Object.assign(novel, input, { updatedAt: now() });
  await writeData(data);
  return novel;
}
...
export async function deleteNovel(novelId: string): Promise<void> {
  ...
  data.chatMessages = data.chatMessages.filter(
    (message) => message.novelId !== novelId,
  );
  for (const [sceneId, state] of Object.entries(data.documentStates)) {
    if (state.novelId === novelId) {
      delete data.documentStates[sceneId];
    }
  }
...
export async function updateChapter(
  ...
  const data = await readData();
  const { novel, chapter } = findChapter(data, chapterId);
...
export async function updateStoryBibleEntry(
  ...
  const data = await readData();
  const { novel, entry } = findStoryBibleEntry(data, entryId);
